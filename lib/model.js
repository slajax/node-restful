...
function preprocess(req, res, next) {
  ...
  var where = (req.query.where) ? req.query.where : {};
  if(typeof where === "string") {
    try{
      where = JSON.parse(where);
    }catch(e) {
      res.locals = { status_code: null, bundle: {} };
      res.locals.status_code = 500;
      res.locals.bundle.status_code = res.locals.status_code;
      res.locals.bundle.message = 'Invalid JSON in where clause';
      res.status(res.locals.status_code).json(res.locals.bundle).send();
    };
  }

  req.quer = this.filter(req, this.find(where));
  if (!('locals' in res)) {
    res.locals = {};
  }
  res.locals.bundle = {};
